...
|In case of Closed Source projects, code security is a concern when a large number of people are working on the same project|CollaboRate provides adequate code security such that a large number of people can work on the same project from any location| 
...
- **Tiny tasks**: It is easy to complete tiny tasks or 'micro-tasks' as compared to larger ones. Also we can easily and accurately measure the total contribution of a person in terms of the total number of such tiny tasks done by that person. In the CollaboRate process, a micro-task is the work done to break down a given problem statement into the bare minimum number of sub-problems (ideally two sub-problems, but in a few cases, three or more), with the help of the Work Breakdown Wizard. 
- **Context-free Tiny Tasks**: For the tasks to be tiny, the user who tries to break down a problem statement, should not have to read the parent problem statement. This must be kept in mind and followed when creating child problem statements. The child problem statement can be created by: first, just copy-pasting the parent problem statement. Then remove sentences and phrases from this copy that are not relevant to the parent problem. Finally, we can elaborate sentences and phrases that are relevant to the child problem. The context-free method of programming is supported by CollaboRate.js, a javascript/node.js library that can be directly used by programmers
- **System of Financial Rewards and Penalties**: For projects that provide a monetary reward to the workers, if the child problem statement is not properly defined by a worker or is not submitted in time, a suitable penaltly is levied on that worker that is proportional to the amount of change needed to correct the problem statement or to compensate for the delay caused by that worker. 

CollaboRate will help make available online intellectual work that involve small 'micro-tasks' of text comprehension (text less than 2000 characters). The text is the description of an software application. Since most people are familiar with the concept of installing apps on smartphones, we also refer to software applications as 'apps'.

The above-mentioned text is the main input for the Work Breakdown Wizard. The wizard asks a series of questions about the text to the worker. The initial questions are objective-type (i.e. the user needs to simply choose from one of the available choices already displayed on the screen). The choice made to answer the previous question, will determine the next question and the next set of choices available to answer that question. In response to the final question, some text usually needs to be captured. In this way, workers break down English text that are descriptions of software apps/sub-apps, into logic parts and text parts. The logic parts are used to generate code. The text parts are used to create new text micro-tasks for the next level of the work breakdown. Eventually the text tasks become so small that they become just logic parts and cannot be further broken into text parts. 

For commercial use, Natural Language Processing and supporting AI techniques will probably be needed to validate the inputs given by the 'crowd'(the workers). The need for such technology can be reduced or even eliminated, by having a suitable cross-worker review and reward process in place. As such, AI techniques for validating the work done by the crowd is out of the scope of this Open Source project. (The project may eventually implement some simple and effective Natural Language Processing-based validations in the Work Breakdown wizard, such as words present in the parent tasks that are missing in both the child tasks etc.) Advanced AI techniques can be implemented by commercial entities called 'CrowdSourcing vendors' that will be organizations like IT Service companies. These companies will take work from customers and create the online text comprehension work opportunities. These vendors act like a broker between customers and the Crowd, and earn a percentage of profit in the process.

CollaboRate defines several activities that comprise the software lifecycle. It enables many of these activities to be [crowdsourced](https://en.m.wikipedia.org/wiki/Crowdsourcing), even in commercial (closed-source) projects. This results in fast project completion time. CollaboRate enables crowdsourcing more effectively than any other known crowdsourcing method. 
...
|Unlimited ability to take it easy| Because of the huge crowd of people available to work on the project, employees can also work as little as they need to (say, due to personal emergencies, or need for vacation) without anyone worrying about impact to the project.|  j 
...
We end the risk caused by the need for skilled workers and project knowledge. This in turn allows mass participation. It provides earning opportunities for even the less capable. Here's how:  
   ...
   - The codebase created by this process and tool, is considered to be in eXtreme Documentation (XD) format. By this we mean that:
     - Each function has the entire task description pasted before it as a header. Or it at least contains a link to the task ID. On clicking the task ID, the task description ideally opens up in a popup window.
     - Each function contains a maximum of only 2 steps of code.  
The codebase is ideally autogenerated from the database of the task tracking system. In future the CollaboRate project may release such a custom task tracking system. Such a codebase facilites the future maintenance of even closed-source projects, under the CollaboRate process.
