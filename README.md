...
The questions are asked by a software tool called a 'Work Breakdown Wizard'. This wizard asks a series of questions to the worker. The initial questions are objective-type (i.e. the user needs to simply choose from one of the available choices already displayed on the screen). The choice made to answer the previous question, will determine the next question and the next set of choices available to answer that question. In response to the final question, some text usually needs to be captured. In this way, workers slowly break down English text that are description of parts of software apps, into logic and text that is used to generate code as well as to create new text micro-tasks for the next level of the work breakdown.   
